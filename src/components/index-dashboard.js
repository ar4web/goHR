...
function renderNeedsAttention() {
  ...
  if (expiring.length) {items.push(`<li class="activity-item"><div class="activity-avatar" style="background:var(--yellow)">◷</div><div><div class="activity-body">${t('dash.iqamaAlert').replace('{n}', expiring.length).replace('{name}', empName(expiring[0]))}</div><div class="activity-time">${t('dash.thisWeek')}</div></div></li>`);}
