...
export function initSettings() {
  ...
  const railState = (() => {
    try { return { org: true, prefs: true, modules: true, data: true, ...JSON.parse(localStorage.getItem(RAIL_STORE) || '{}') }; }
    catch (_e) { return { org: true, prefs: true, modules: true, data: true }; }
  })();
  const paintRail = () => {
...
${logo}<span style="font-size:14px;font-weight:bold;color:${accent}">${esc(nmA)}</span>${nmB ? `<br><span style="color:${mut}">${esc(nmB)}</span>` : ''}<br><span style="color:${mut}">${crLbl} ${esc(s.crNo)} · ${vatLbl} ${esc(s.taxNo)}</span><br><span>${contact}</span></td></tr></table>`;
  ...
  document.getElementById('set-open-cz')?.addEventListener('click', () => {
