...
import { NAV, SETTINGS_NAV } from './shell-render.js';
...
function buildItems() {
  const out = [];
  // Every navigable leaf: sidebar groups plus the footer Settings sections.
  NAV.forEach((group) => {
  ...
  SETTINGS_NAV.forEach((sec) => {
    for (const leaf of collectLeaves(sec.items, [])) {
      out.push({
        kind: 'page',
        label: leaf.text,
        section: sec.section,
        href: leaf.href,
        keywords: `${leaf.text} ${sec.section} ${leaf.key || ''}`.toLowerCase()
      });
    }
  });
  // Inline actions
